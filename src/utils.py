...
def deduce_position(word: Word) -> None:
    ...
    root_indices = [i for i, m in enumerate(morphs) if m.morph_type == MorphType.ROOT]

    if not root_indices:
        root_indices = [0]  # Assume the first morph is the root
    ...
    if len(root_indices) == 1:
        ...
        for i in range(len(morphs)):
            m = morphs[i]
            m.morph_position = (
                MorphPosition.ROOT if i == root_index
                else MorphPosition.PREFIX if i < root_index
                else MorphPosition.SUFFIX
            )
    else:
        ...
        # Assign PREFIX
        for i in range(first_root):
            morphs[i].morph_position = MorphPosition.PREFIX

        # Assign ROOT
        for i in root_indices:
            morphs[i].morph_position = MorphPosition.ROOT

        # Assign INTERFIX
        if last_root - first_root > 1:
            root_set = set(root_indices)
            for i in range(first_root + 1, last_root):
                if i not in root_set:
                    morphs[i].morph_position = MorphPosition.INTERFIX

        # Assign SUFFIX
        for i in range(last_root + 1, len(morphs)):
            morphs[i].morph_position = MorphPosition.SUFFIX
...
def load_annotations(filepath: str, indent: int = 4) -> List[DataSentence]:
    ...
    def flush_word():
        ...
        if current_word is not None:
            deduce_position(Word(current_word))
